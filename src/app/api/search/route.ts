...
import { customerSearchFilters } from "@/modules/customers/services/customer-service";
...
export async function GET(request: Request) {
  try {
    ...
    const [customers, bikes, workOrders, inventory] = await prisma.$transaction([
      prisma.customer.findMany({ where: { workshopId: context.workshopId, active: true, OR: customerSearchFilters(q) }, select: { id: true, name: true, phone: true }, take: 5 }),
