...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string; lineId: string }> }) {
  try { const { id, lineId } = await params; return Response.json({ data: await workOrderItemService.removePart(await getRequestContext(), id, lineId) }); }
  catch (error) { return errorResponse(error); }
}

export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; lineId: string }> }) {
  try { const { id, lineId } = await params; return Response.json({ data: await workOrderItemService.updatePart(await getRequestContext(), id, lineId, updateWorkOrderLineSchema.parse(await request.json())) }); }
  catch (error) { return errorResponse(error); }
}
