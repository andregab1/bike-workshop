...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; quoteId: string }> }) {
  try { const { id, quoteId } = await params; const input = quoteDecisionSchema.parse(await request.json()); return Response.json({ data: await workOrderQuoteService.decide(await getRequestContext(), id, quoteId, "APPROVED", input) }); }
